...
# MySQL DB URL - Update with your credentials
...
def prepare_ncf_data(interactions_df, content_df):
    ...
    try:
        ...
        logger.error(f"Error preparing NCF  {str(e)}")
        raise
...
def load_and_preprocess_data(db_url: str):
    ...
    try:
        logger.info("Loading and preprocessing data from MySQL...")
        engine = create_engine(db_url)

        with engine.connect() as conn:
            # Load content (posts)
            content_df = pd.read_sql(text("SELECT id AS content_id, title FROM posts"), con=conn)

            reactions_df = pd.read_sql(text("""
                SELECT user_id, post_id AS content_id, emoji AS reaction_type
                FROM post_reactions
            """), conn)


            favorites_df = pd.read_sql(text("""
                SELECT fav.user_id, fav.post_id AS content_id
                FROM favorites fav
                JOIN posts p ON fav.post_id = p.id
                JOIN users u ON fav.user_id = u.id
            """), con=conn)

            comments_df = pd.read_sql(text("""
                SELECT author_id AS user_id, post_id AS content_id
                FROM comments
            """), con=conn)


            shares_df = pd.read_sql(text("""
                SELECT user_id, post_id AS content_id
                FROM share
            """), con=conn)

            reported_df = pd.read_sql(text("""
                SELECT reporter_id AS user_id, post_id AS content_id
                FROM reported_posts
            """), con=conn)
        ...
        # Map engagement types to weights
        engagement_weights = {
            'like': 2.0,
            'love': 3.0,
            'wow': 1.5,
        ...
        # Add engagement weight column
        reactions_df['engagement_weight'] = reactions_df['reaction_type'].map(engagement_weights).fillna(1.0)
        reactions_df['type_engagement'] = 'reaction'

        favorites_df['engagement_weight'] = 4.0
        favorites_df['type_engagement'] = 'favorite'

        comments_df['engagement_weight'] = 4.0
        comments_df['type_engagement'] = 'comment'

        shares_df['engagement_weight'] = 5.0
        shares_df['type_engagement'] = 'share'

        reported_df['engagement_weight'] = -2.0
        reported_df['type_engagement'] = 'report'

        # Combine all interactions
        interactions_df = pd.concat([
            reactions_df[['user_id', 'content_id', 'engagement_weight']],
            favorites_df[['user_id', 'content_id', 'engagement_weight']],
            comments_df[['user_id', 'content_id', 'engagement_weight']],
            shares_df[['user_id', 'content_id', 'engagement_weight']],
            reported_df[['user_id', 'content_id', 'engagement_weight']]
        ])
...
def main():
    try:
        ...
        content_df, interactions_df = load_and_preprocess_data(DB_URL)
